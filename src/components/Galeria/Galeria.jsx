...
const Galeria = ({ idioma }) => {
  ...
  function visibleImageFn(n) {
    if (visibleImages[2] === n) {
      return 'w-[90%] mx-auto max-w-[600px] xl:absolute xl:left-1/2 xl:translate-x-[-300px] transform duration-500';
    }
    if (visibleImages[1] === n) {
      return 'hidden xl:block xl:absolute xl:left-1/2 xl:translate-x-[-937.5px] transform duration-500';
    }
    if (visibleImages[0] === n) {
      return 'hidden xl:block xl:absolute xl:left-1/2 xl:translate-x-[-2550px] transform duration-500';
    }
    if (visibleImages[3] === n) {
      return 'hidden xl:block xl:absolute xl:left-1/2 xl:translate-x-[337.5px] transform duration-500';
    }
    if (visibleImages[4] === n) {
      return 'hidden xl:block xl:absolute xl:left-1/2 xl:translate-x-[1950px] transform duration-500';
    }
    return 'hidden';
  }
  ...
  return (
    <section className="bg-gray xl:overflow-x-hidden">
      <h2
        className={`mx-auto max-w-[90%] pt-12 pb-20 text-center text-6xl font-light leading-[4rem] tracking-wide  md:text-7xl xl:max-w-[1120px]  `}>
        <span className="text-two">{titulo.titulo1} </span>
        <span className="text-one">{titulo.titulo2}</span>
      </h2>

      <div className="mb-8 h-fit w-full   xl:relative xl:h-[450px]">
        {images.map((img) => {
          return (
            <picture key={img}>
              <source
              ...
              <img
                ...
                alt=""
                className={` w-[90%] max-w-[600px] rounded-xl  ${visibleImageFn(
                  img
                )}`}
              />
            </picture>
